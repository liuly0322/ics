...
export default (function () {
  ...
  function toHexString(number) {
    var padLength =
      arguments.length <= 1 || arguments[1] === undefined ? 4 : arguments[1]
    var prefix =
      arguments.length <= 2 || arguments[2] === undefined ? 'x' : arguments[2]
  ...
  function parseLiteral(text) {
    ...
    var toParse = isDecimal
      ? text.substring(negate ? 2 : 1)
      : negate
      ? first + text.substring(2)
      : text
  ...
  function parseString(text) {
    ...
    if (text.charAt(0) !== quote || text.charAt(text.length - 1) !== quote) {
      error(
        'the string needs to start and end with ' +
  ...
  function findOrig(tokenizedLines) {
    ...
    if (lineNumber === -1) {
      throw new Error(
        "Looks like your program's empty! " +
    ...
    if (operands !== 1) {
      throw new Error(
        'The .ORIG directive expects exactly one operand, ' +
    ...
    var operand = line[1]
    var orig = withContext(
      parseLiteral,
      'while parsing .ORIG directive operand'
    )(operand)
        ...
        if (existingLocation !== undefined) {
          ...
          return _extends({}, state, {
            symbols: _extends(
              {},
              state.symbols,
              _defineProperty({}, labelName, state.address)
            ),
          })
        }
      },
  ...
  function encodeInstruction(tokens, pc, symbols) {
    ...
    if (upname === 'ADD' || upname === 'AND') {
      ...
      var sr2OrImm = asLiteral.success
        ? 32 | inBits(asLiteral.result, 5, 'immediate field')
        : 0 | parseRegister(last)
  ...
  function reduceProgram(lines, begin, handlers, initialState) {
    ...
    var handleLabel =
      _handlers$handleLabel === undefined ? id : _handlers$handleLabel
    var _handlers$handleDirective = handlers.handleDirective
    var handleDirective =
      _handlers$handleDirective === undefined ? id : _handlers$handleDirective
    var _handlers$handleInstruction = handlers.handleInstruction
    var handleInstruction =
      _handlers$handleInstruction === undefined
        ? id
        : _handlers$handleInstruction
    ...
    var commands = [].concat(
      _toConsumableArray(trapVectors),
      instructions,
      directives
    )
